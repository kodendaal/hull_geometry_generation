...
def scale_xyz_coordinates(input_array):
    ...
    x_min = x.min()
    x_max = x.max()
    # x_scaled = (x - x_max) / (x_min - x_max)
    x_scaled = 2 * (x - x_min) / (x_max - x_min) - 1  # Scaling to [-1, 1]

    # Calculate the scale factor
    scale_factor = np.where(x != 0, np.abs(x_scaled / x), 0)

    # Apply the same scale factor to y and z
    x_scaled = x_scaled * (-1)
    ...
    if embedding.shape[1] > 2: 
        ...
        for i, class_name in enumerate(v_class):
            ...
            plt.scatter(
                ...
                c=color_map[class_name], label=class_name, s=100, alpha=0.8
            )
...
def main():
    ...
    folder_path = r'P:\70xxx\701xx\7018x\70180\RD\301 - HullGAN\Encode\NSS'

    # Load the geometries
    geometries, labels = load_geometries_from_folder(folder_path)
    ...
    print(f"Loaded {len(geometries)} geometries from the folder.")
